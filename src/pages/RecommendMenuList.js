...
const RecommendMenuList = () => {
  ...
  const [sortOption, setSortOption] = useState('최신 게시글');
   ...
   const {fetchRecommendData, recommendRecipes, setRecommendRecipes, loading} = useRecipe();
  ...
  if (loading) {
    ...
    console.log("MyMyMyMyMyMyMy post recipes ====================" , recommendRecipes);
  ...
  // 검색어로 필터링된 아이템
  const filteredAndSortedItems = recommendRecipes.filter(item => {
